 ...
 * Maps incoming requests to child services and queries.
 */
public abstract class ServletService extends HasServiceContext implements BiConsumer<HttpServletRequest, HttpServletResponse> {
    ...
    protected ServletService(ServletServiceContext context) {
        super(context);
        logger.info("started construction");
        notFoundRedirecter = new SimpleForwarder(context).withUrl(url("error404Jsp"));
        errorRedirecter = new SimpleForwarder(context).withUrl(url("error500Jsp"));
